...
// blobData dengan { private: true } - JANGAN diubah jadi public. Lihat
// catatan di lib/blobData.js & lib/activityLog.js soal kenapa.
...
async function loadExtraAdminUsers() {
    const extra = await readJson(ADMIN_EXTRA_KEY, [], { private: true });
...
async function createAdminAccount(username, password, createdBy, options) {
    ...
    await writeJson(ADMIN_EXTRA_KEY, extra, { private: true });
...
async function deleteAdminAccount(username) {
    ...
    await writeJson(ADMIN_EXTRA_KEY, remaining, { private: true });
}
...
async function updateAdminAccount(username, options, editedBy) {
    ...
    if (isOwnerUsername(cleanUsername)) {
        throw new Error("Akun owner (azriel/david) tidak bisa diubah lewat panel.");
    }
