...
const SUPER_ADMIN_USERNAMES = ["azriel", "david"];

// =========================
// Sistem role:
// - super_admin : akun tetap azriel & david. Akses penuh, tidak bisa dihapus.
...
//                 sudah dipilihkan oleh super_admin saat akun dibuat. Tidak bisa
...
function isSuperAdminUsername(username) {
    return SUPER_ADMIN_USERNAMES.includes(String(username || "").toLowerCase());
}

// Role "sebenarnya" dari sebuah akun: super_admin selalu dipaksa jadi super_admin
// apapun yang tersimpan (jaga-jaga kalau data lama tidak punya field role).
function resolveRole(user) {
    if (isSuperAdminUsername(user && user.username)) return ROLES.SUPER_ADMIN;
...
// Password akun super_admin (azriel/david) yang sudah pernah diganti lewat panel.
// Akun super_admin sendiri hardcode di DEFAULT_ADMIN_USERS/ADMIN_USERS (kode/env),
// jadi passwordHash barunya TIDAK bisa ditulis balik ke sana. Sebagai
// gantinya, kalau super_admin ganti password, hash barunya disimpan terpisah di
// sini (kvStore) dan dipakai untuk "menimpa" passwordHash bawaan saat login
// & verifikasi. Key: username super_admin (lowercase) -> { passwordHash, updatedAt, updatedBy }.
const SUPER_ADMIN_PASSWORD_KEY = "admin-super-admin-passwords.json";
...
async function loadSuperAdminPasswordOverrides() {
    const overrides = await readJson(SUPER_ADMIN_PASSWORD_KEY, {});
...
// dengan passwordHash super_admin ditimpa kalau sudah pernah diganti lewat panel)
// + tambahan.
async function loadAllAdminUsers() {
    const base = loadBaseAdminUsers();
    const overrides = await loadSuperAdminPasswordOverrides();
    const baseWithSuperAdminOverrides = base.map(u => {
        const override = isSuperAdminUsername(u.username)
    ...
    return [...baseWithSuperAdminOverrides, ...extra];
}

// Daftar ringkas untuk ditampilkan di panel "Kelola Akun Admin"
// (tanpa passwordHash). Menandai mana yang super_admin (azriel/david, tak bisa dihapus)
...
async function listAdminAccounts() {
    ...
    const extraList = extra.map(u => ({
        ...
        removable: !isSuperAdminUsername(u.username),
...
// bahwa yang membuat adalah super_admin — lihat api/admin/admins.js).
...
async function createAdminAccount(username, password, createdBy, options) {
    ...
    if (isSuperAdminUsername(cleanUsername)) {
        throw new Error("Username tersebut sudah dipakai untuk akun super_admin.");
    }
...
// Hapus akun admin tambahan (tidak bisa menghapus akun bawaan/super_admin).
async function deleteAdminAccount(username) {
    const cleanUsername = String(username || "").trim().toLowerCase();
    if (isSuperAdminUsername(cleanUsername)) {
        throw new Error("Akun super_admin (azriel/david) tidak bisa dihapus.");
    }
...
async function updateAdminAccount(username, options, editedBy) {
    ...
    // super_admin (lihat api/admin/admins.js) — jadi otomatis hanya azriel/david
    // yang bisa memicu perubahan ini.
    if (isSuperAdminUsername(cleanUsername)) {
        return updateSuperAdminPassword(cleanUsername, opts.newPassword, options, editedBy);
    }
    ...
    if (hasNewUsername) {
        ...
        if (isSuperAdminUsername(newUsername)) {
            throw new Error("Username tersebut sudah dipakai untuk akun super_admin.");
        }
...
// Ubah password akun super_admin (azriel/david). Username super_admin TIDAK bisa diubah
...
// setelah pemanggil diverifikasi super_admin (lihat api/admin/admins.js).
async function updateSuperAdminPassword(username, newPassword, options, editedBy) {
    const opts = options || {};
    const cleanUsername = String(username || "").trim().toLowerCase();

    const hasNewUsername = opts.newUsername !== undefined && opts.newUsername !== null && String(opts.newUsername).trim() !== "";
    if (hasNewUsername && String(opts.newUsername).trim().toLowerCase() !== cleanUsername) {
        throw new Error("Username akun super_admin (azriel/david) tidak bisa diubah, hanya password.");
    }

    const hasNewPassword = newPassword !== undefined && newPassword !== null && String(newPassword) !== "";
    if (!hasNewPassword) {
        throw new Error("Isi password baru untuk mengubah akun super_admin.");
    }
    ...
    await writeJson(SUPER_ADMIN_PASSWORD_KEY, overrides);
